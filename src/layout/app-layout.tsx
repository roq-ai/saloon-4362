...
import { FiMail, FiUsers, FiMenu, FiUser, FiMessageCircle, FiFile, FiBox, FiScissors } from 'react-icons/fi';
...
const SidebarContent = ({ onClose, ...rest }: SidebarProps) => {
  ...
  const { data, error, isLoading } = useSWR<SalonInterface[]>(
    ...
    () => getSalons({ tenant_id: session?.user?.tenantId }).then(({ data }) => data),
  );
  loading = isLoading;
